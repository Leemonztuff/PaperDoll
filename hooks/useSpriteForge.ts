...
function forgeReducer(state: AppState, action: ForgeAction): AppState {
  switch (action.type) {
    case "SET_BASE_IMAGE":
      return { ...state, baseImage: action.payload, activeParent: null }
    ...
    case "UPDATE_BG":
      return {
        ...
        config: {
          ...state.config,
          protocols: { ...state.config.protocols, backgroundStyle: action.payload },
        },
      }
    case "TOGGLE_NODE":
      return {
        ...
        config: {
          ...state.config,
          neuralChain: state.config.neuralChain.map((n) =>
            n.id === action.payload ? { ...n, isActive: !n.isActive } : n
          ),
        },
      }
    default:
...
export function useSpriteForge() {
  ...
  const getActiveApiKey = () => {
    const key = ApiKeyService.getCurrentKey()
    if (!key) {
  ...
  const generateMannequin = useCallback(async () => {
    ...
    try {
      ...
      const mannequin = await ImageService.generateBaseMannequin(
        provider,
        apiKey,
        state.config
      )
  ...
  const executeBaseExtraction = useCallback(
    async (imageUrl: string) => {
      ...
      try {
        const provider = getActiveProvider()
        const apiKey = getActiveApiKey()
        const baseDNA = await ImageService.extractBaseDNA(
          provider,
          apiKey,
          imageUrl,
          state.config
  ...
  const executeSynthesis = useCallback(
    async (prompt: string) => {
      ...
      try {
        const provider = getActiveProvider()
        const apiKey = getActiveApiKey()
        const url = await ImageService.synthesizeEvolution(
          provider,
          apiKey,
          state.baseImage,
